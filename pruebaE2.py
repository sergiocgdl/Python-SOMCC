...
from somE2 import SCCSOM_EAC_SL
...
for i in range(len(sets)):
    ...
    ejec = np.zeros(10)
    for j in range(10):
        som = SCCSOM_EAC_SL(shape, seed=j)  # initialize a 5 by 5 SOM
        som.initialize(pobl, restr)
        som.fit(restr, ep, save_e=True, decay="")  # fit the SOM for 10000 epochs, save the error every 100 step
