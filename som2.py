...
class SOM2(object):
    ...
    def calcularAumentoInfeasibility(self, data_neurona, ind, restr):
        infeasibility = 0
    ...
    def winnerRestr(self, vector1, vector2, restr):
        ...
        if(restr==1.0):
            ...
            while(d2 and distanceEu(self.map[d2[0][1]], self.map[d1[0][1]]) < 3*self.std):
                d2.pop(0)
    ...
    def fit(self, data, restr, epochs=0, save_e=False, decay='hill'):
        ...
        if save_e:  # save the error to history every "interval" epochs
            ML, CL, WL = listaRestricciones(restr)
            for i in range(epochs):
                ...
                self.distICEpoch[self.epoch] = self.calcularDistanciaICTotal(data, mode="som")
    ...
    def plot_labeled_map(self, data, targets, n_targets, filename=None, colors=None, markers=None):
        ...
        if filename:
            ...
            plt.close()
            print("Clustering map plot done!")
        else:
            plt.show()
    ...
    def plot_inf_history(self, color='orange', filename=None):
        ...
        if filename:
            plt.savefig(filename)
            plt.close()
            print("Infeasibility history plot done!")
        else:
            plt.show()
            
    def plot_acc_history(self, color='orange', filename=None):
        if not len(self.historyAcc):
            raise LookupError("No accuracy history was found! Is the SOM already trained?")
        fig, ax = plt.subplots()
        ax.plot(range(0, self.epoch), self.historyAcc, '-o', c=color)
        ax.set_title('SOM Accuracy History', fontweight='bold')
        ax.set_xlabel('Epoch', fontweight='bold')
        ax.set_ylabel('Accuracy', fontweight='bold')
        if filename:
            plt.savefig(filename)
            plt.close()
            print("Accuracy history plot done!")
        else:
            plt.show()
        
    def build_map_winner(self, data):    
        self.clusters=[]
            ...
            while(not existeVecino):
                for i in range(len(self.clusters)):
                    for j in range(i+1, len(self.clusters)):
                            if(vecino(self.clusters[i], self.clusters[j], dist)):
                                existeVecino = True
                                inf = self.calcularAumentoInfeasibilityIC(restr, self.clusters[i].inputs, self.clusters[j].inputs)
                                if(inf <= mejor_inf):
    ...
    def calcularAumentoInfeasibilityIC(self, restr, cl1, cl2):
        inf = 0
        for i in cl1:
            for j in cl2:
                if(restr[i,j]==1):
    ...
    def calcularDistanciaICTotal(self, data, mode="cluster"):
        d = 0.0
        if(mode=="som"):
            for i in range(self.numNeuronas):
                self.centroides[i]=np.zeros(len(data[0]))
