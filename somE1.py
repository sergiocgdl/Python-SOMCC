...
class SCCSOM_E(object):
    ...
    def build_map_clusters(self):    
        ...
        for i in range(self.numNeuronas):
            self.variance[i]=0
            for j in range(len(self.winners[i])):
                d = distanceEu(self.pobl[self.winners[i][j]], self.centroides[i])
                self.variance[i]+=d
    ...
    def buildCertainly(self):
        certainly = np.zeros(len(self.pobl))
        for i in range(len(self.pobl)):
            suma = 0
            for j in range(len(self.pobl)):
                if(i!=j):
                    f = self.coasociacion[i][j]
                    if(f <= 0.5):
                        suma += 1-2*f
                    else:
                        suma += 2*f-1
            suma /= len(self.pobl)-1
            certainly[i] = suma   
        self.certainly = certainly
        
    def sortPobl(self):
        orden = np.argsort(self.certainly)
        orden = orden[::-1]

        poblOrdenada = []
        for i in orden:
            poblOrdenada.append(self.pobl[i])
